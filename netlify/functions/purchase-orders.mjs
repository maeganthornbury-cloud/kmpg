...
function renderPoPrint(po) {
  ...
  const isInternalPo = String(po.poType || "").toLowerCase() === "internal";
  ...
  const shopNotesSection = isInternalPo && String(po.shopNotes || "").trim()
    ? `<div class="box" style="margin-top:12px;"><b>SHOP NOTES</b><br/>${escapeHtml(po.shopNotes || "")}</div>`
    : "";

  return `<!doctype html>
<html><head><meta charset="utf-8" /><title>${isInternalPo ? "Internal PO" : "Purchase Order"} ${escapeHtml(po.poNumber || "")}</title>
<style>
